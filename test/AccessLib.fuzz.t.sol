...
contract AccessLibFuzzTest is Test {
    function testFuzz_computeExpiry_durationZero_alwaysReturnsMax(uint256 currentExpiry, uint256 nowTs) public {
        uint256 expiry = AccessLib.computeExpiry(currentExpiry, nowTs, 0);
        assertEq(expiry, type(uint256).max);
    }

    function testFuzz_computeExpiry_newPurchase_startsFromNow(uint256 nowTs, uint256 duration) public {
    ...
    function testFuzz_isValid_neverPurchased_alwaysFalse(uint256 nowTs) public {
        bool valid = AccessLib.isValid(0, nowTs);
        assertFalse(valid);
    }

    function testFuzz_isValid_permanentAccess_alwaysTrue(uint256 nowTs) public {
        bool valid = AccessLib.isValid(type(uint256).max, nowTs);
        assertTrue(valid);
    }

    function testFuzz_isValid_beforeExpiry_returnsTrue(uint256 expiry, uint256 nowTs) public {
    ...
    function testFuzz_isValid_afterExpiry_returnsFalse(uint256 expiry, uint256 nowTs) public {
    ...
    function testFuzz_isValid_atExpiry_returnsTrue(uint256 expiry) public {
